...
export function generateObjectionDocument(data: NotarialData): string {
  ...
  // Format creditor name for display
...
1. В соответствии с п.2 ст.92-1 Закона РК «О нотариате» на основании исполнительной надписи или соответствующего постановления производится взыскание задолженности по следующим бесспорным требованиям, в том числе об исполнении обязательства, основанного на письменной сделке, срок исполнения которой наступил и неисполнение обязательства признается должником, в том числе в ответе на претензию, направленную взыскателю в порядке досудебного урегулирования спора. В моем случае требование ${creditorDisplay} я не согласна с установленной суммой задолженности. Также мной не был получен досудебная претензия и неисполнение обязательства я не признавал (-а).

2. Согласно п. 3 ст. 92-1 Закона на основании исполнительной надписи не производится взыскание неустоек (пеней), процентов. Однако, Вами учинена исполнительная надпись на сумму задолженности в размере ${data.totalAmount} тенге${data.totalAmountWords ? ` (${data.totalAmountWords})` : ''}, включив проценты (вознаграждение), неустойку (пени) в нарушение требований п. 3 ст. 92-1 Закона.
